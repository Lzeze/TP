...
//检测一行字符串最后有个字符是否是分号，是分号则一条sql语句结束，否则sql还有一部分在下一行中
    ...
    public function delSqlFiles() {
        if (IS_POST) {
            ...
            echo json_encode(array("status" => 1, "info" => "已删除：" . implode("、", $files), "url" =>U("SysData/restore?" . time())));
        }
    }
...
//发送邮件
    ...
    public function delZipFiles() {
        if (IS_POST) {
            ...
            echo json_encode(array("status" => 1, "info" => "已删除：" . implode("、", $files), "url" =>U("SysData/zipList?" . time())));
        }
    }
